  ...
  return (
    <div className="liquid-glass rounded-3xl p-4 text-slate-200 select-none flex flex-col font-sans text-xs shadow-2xl">
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-2.5 mb-2">
        ...
        <span className="text-[10px] text-slate-300 font-mono font-semibold px-2 py-0.5 rounded-full bg-white/10 border border-white/15">
          ZNY HIGH BAY
        </span>
      </div>
            ...
            return (
              <div
                ...
                className={`p-2.5 rounded-2xl transition-all duration-200 cursor-pointer text-xs ${
                  isSelected
                    ? 'liquid-glass-blue border-sky-400/60 shadow-[0_0_15px_rgba(56,189,248,0.25)]'
                    : isCritical
                    ? 'liquid-glass-red border-red-500/50 hover:brightness-110'
                    : isCaution
                    ? 'liquid-glass-amber border-amber-500/50 hover:brightness-110'
                    : 'liquid-glass-subtle hover:liquid-glass-active border-white/5 hover:border-white/25'
                }`}
              >
                ...
                <div className="flex items-center justify-between mb-1.5 font-mono">
                  ...
                  {isCritical ? (
                    <span className="flex items-center gap-1 text-[9px] font-bold text-red-100 bg-red-500/25 px-2 py-0.5 rounded-full border border-red-400/40 animate-pulse">
                      <Flame className="w-2.5 h-2.5 text-red-400" />
                      STCA
                    </span>
                  ) : isCaution ? (
                    <span className="flex items-center gap-1 text-[9px] font-bold text-amber-100 bg-amber-500/25 px-2 py-0.5 rounded-full border border-amber-400/40">
                      <AlertTriangle className="w-2.5 h-2.5 text-amber-400" />
                      CONFLICT
                    </span>
                  ) : (
                    <span className="text-[9px] text-slate-300 font-semibold px-2 py-0.5 rounded-full bg-white/10 border border-white/15">
                      NOMINAL
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-4 gap-1.5 text-[10px] bg-black/40 p-1.5 rounded-xl border border-white/5 font-mono">
                  ...
                  <div>
                    ...
                    <span className="text-white font-semibold">{ac.squawk ?? '1200'}</span>
                  </div>
                </div>
              </div>
            );
